...
- 用户确认 MI 6（`9688fa60`）和 Xiaomi 13（`efcb9031`）均已重新上线，当前未连接蓝牙设备；本节取代 2026-07-12 的 MI 6-only 执行条件。
- 本轮提交门槛恢复为双机安装/启动、前台服务、P2P 身份与信令、后台重开、停止释放和立即重启客观检查。
- 无蓝牙设备时，两台均须客观确认安全回退手机音频；蓝牙通信路由和所有依赖人耳判断的项目记录为“未执行”，不得表述为已通过，且不阻塞本轮提交。

### 2026-07-12 当前设备矩阵修订

- 用户确认当前仅 MI 6（`9688fa60`）连接本机，且未连接蓝牙设备。
- 本轮提交门槛改为：自动单测、Lint、编译，以及 MI 6 的安装/启动、前台服务、P2P、停止释放和立即重启客观检查。
- 双机发现/信令、Xiaomi 13 蓝牙路由和所有依赖人耳判断的项目记录为“未执行”，不得表述为已通过；这些项目不阻塞本轮提交。

### 2026-07-11 执行条件补充

- MI 6（`9688fa60`）和 Xiaomi 13（`efcb9031`）现均在线；Xiaomi 13 已连接蓝牙耳机。
- 自动单测、Lint、编译、双机安装/启动、服务状态、P2P/信令、蓝牙客观路由和停止释放检查均是硬门槛。
- 双机实际听说和其他依赖人耳判断的项目由用户在最终验收时确认；未获人工确认前记录为“未执行”，不得表述为已通过。
