...
- 当前只连接 MI 6（`9688fa60`），且没有连接蓝牙耳机。
- 自动单测、Lint、编译、MI 6 安装/启动、服务状态和停止释放检查仍是硬门槛。
- 本轮跳过 Xiaomi 13、双机互听、蓝牙耳机听说和其他依赖主观听感的验收；这些项目记录为“未执行”，不判失败。
- 后续具备双机和蓝牙条件时，再补做原完整真机矩阵；本轮不得把跳过项表述为已通过。
