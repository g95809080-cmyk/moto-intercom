...
摩声是一款 Android 骑行对讲 App。打开 Wi-Fi，找到附近车友，连上后直接说话——不用一直按着按钮，也不需要互联网。本分支在原有双人模式之外，新增了含房主最多四人的离线房间入口。
...
当前正式发布的 APK 是**一对一对讲**；四人功能仍在实现分支验证，尚未发布。仓库里的 iOS 代码仍在开发。
...
代码和自动化验证记录见本分支的验证文档。当前没有四台实体设备，四人互听、无路由器射频和长时稳定性验收尚未执行。
