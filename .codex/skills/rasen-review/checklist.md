...
## Smell Baseline — Fowler's 12 (Standards axis)

<!-- adapted from mattpocock/skills (MIT, Copyright Matt Pocock) -->

On top of everything this repo documents, the Standards axis always carries the **smell baseline** below — a fixed set of Fowler code smells (_Refactoring_, ch.3) that applies even when the repo documents nothing about a given point. Two rules bind it:
