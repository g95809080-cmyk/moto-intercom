...
- 未完成认证不占位；已验证 JOIN 成功即名单准入，重复 request 缓存30秒/128项，未过期缓存满则 BUSY 拒绝新请求而不驱逐撤销凭证；抽象认证证明有效期最多10秒，过期拒绝；移除/离开/结束撤销旧凭据资格。保留期60秒从首次掉线开始；到期保留可恢复的等待记录但不占席位。
- 每条媒体 link 保存两端成员 incarnation、自身全房间单调递增代次及双方确认，替换要求expected旧link lease和两端当前成员lease；换成员资格/掉线清除相关证据，无关成员变化不影响其余关系。所有音频就绪证据只作为模型输入，实际采集/统计探针为后续任务。
