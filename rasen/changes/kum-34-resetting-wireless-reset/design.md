...
- **A Wi-Fi Direct API action can fail, stay BUSY, or never callback.** -> Each close step owns a cancellable timeout, advances once on callback, failure, throw, or timeout, ignores every late duplicate completion, and invalidates delayed BUSY retries when the step advances.
