...
7. **Use layered deterministic evidence.** Coordinator tests cover first/second retries, third-failure reset, target/plan preservation, fresh IDs/deadlines, count exclusions, success reset, and stale callbacks. Orchestrator/reducer tests cover exact reset completion and stop supersession. Resource tests cover one-shot cleanup/restart ordering, and emulator evidence exercises visible `RESETTING`, cleanup/rebuild, and process-safe callback rejection where the platform supports it.
...
- **A Wi-Fi Direct API action can fail or never callback.** -> Existing close actions continue on reported failure and BUSY retries are bounded. The reset completion path remains generation-gated and test evidence must expose any callback gap as P1.
