...
- Reuse the production Wi-Fi Direct close chain (`cancelConnect` -> `removeGroup` -> `clearServiceRequests` -> `clearLocalServices` -> channel `close`) and close LAN/NSD/UDP/Socket/delayed work before rebuilding discovery components.
