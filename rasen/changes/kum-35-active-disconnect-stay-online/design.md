...
2. **Emit one exact attempt-release effect.** A new immutable `ReleaseActiveSessionAndContinueDiscovery(attempt)` effect follows exact control-channel close effects. The effect itself is the Coordinator-issued intentional-disconnect authorization. While the same runtime is running, Service always executes its exact timer, signaling, media, and adapter cleanup; only the connection-state clearing and searching-status publication require the product to remain in the same runtime's idle `DISCOVERING` state with empty Coordinator attempt/channel/pending ownership. The first-terminal ledger deliberately remains `SUCCESS` for an already connected attempt, while pre-connect/recovery cancellation records `CANCELED`; no second mutable terminal authority is introduced. Exact immutable identity checks prevent a delayed old cleanup from touching replacement media or transport leases, and the split finalization gate prevents it from overwriting replacement status.
...
- **A delayed release effect could overlap a replacement attempt.** -> Always drain the Coordinator-authorized old exact cleanup before later transport-open effects, use immutable identity checks in media and adapters, and gate only UI/product finalization on same-runtime idle `DISCOVERING` with empty current ownership.
